...
var (
	...
	address   = fmt.Sprintf("127.0.0.1:%d", 8888)
	liveclass = &registry.ServiceInstance{
		ID:        "liveclass",
		Name:      "liveclass",
		Version:   "1.0.0",
...
func new() *fixedDiscovery {
	return &fixedDiscovery{}
}

// 根据 serviceName 直接拉取实例列表
func (fdis *fixedDiscovery) GetService(ctx context.Context, serviceName string) ([]*registry.ServiceInstance, error) {
	return []*registry.ServiceInstance{liveclass}, nil
}

// 根据 serviceName 阻塞式订阅一个服务的实例列表信息
func (fdis *fixedDiscovery) Watch(ctx context.Context, serviceName string) (registry.Watcher, error) {
	w := &watcher{}
	return w, nil
}
